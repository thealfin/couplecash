...
async function login() {
  ...
  try {
    ...
    if (res.success) {
      ...
      errorMessage.value = res.message || 'Login gagal, periksa email & password Anda'
    }
  } catch (err: any) {
    isLoading.value = false
    errorMessage.value = err?.message || 'Login gagal, periksa email & password Anda'
  }
}
    ...
    <div class="auth-content">
      ...
      <div class="hero-illustration">
        <div class="icon-circle">
          <span class="material-symbols-outlined hero-icon">favorite</span>
        </div>
      </div>
      ...
      <div class="auth-header">
        <h1 class="auth-title">
          Keuangan Kita,
          <span class="auth-gradient-text">Satu Misi.</span>
        </h1>
        <p class="auth-subtitle">
          Kelola uang bersama jadi lebih hangat, transparan, dan tanpa drama.
        </p>
      </div>
      ...
      <div class="form-box">
        ...
        <div class="input-group">
          ...
          <div class="input-field">
            ...
            <input type="email" v-model="email" placeholder="Masukkan Email Anda" class="text-input" />
          </div>
        </div>
        ...
        <div class="input-group">
          ...
          <div class="input-field">
            ...
            <input :type="showPassword ? 'text' : 'password'" v-model="password" placeholder="Masukkan kata sandi" class="text-input" @keyup.enter="login" />
        ...
        <button class="secondary-btn" @click="router.push('/auth/register')">
...
.auth-container {
  ...
  padding:24px 16px; position:relative; overflow:hidden; background:var(--background);
}

.bg-blob { position:absolute; border-radius:50%; filter:blur(60px); pointer-events:none; opacity:0.35; }
...
.auth-content { max-width:400px; width:100%; display:flex; flex-direction:column; align-items:center; position:relative; z-index:1; }

.hero-illustration { margin-bottom:24px; }
.icon-circle { width:88px; height:88px; border-radius:50%; background:var(--primary-fixed); display:flex; align-items:center; justify-content:center; box-shadow:0 8px 24px rgba(70,72,212,0.2); }
.hero-icon { font-size:44px; color:var(--primary); font-variation-settings:'FILL' 1; }

.auth-header { text-align:center; margin-bottom:24px; }
.auth-title { margin:0 0 8px; font-size:28px; font-weight:800; color:var(--on-background); line-height:1.2; letter-spacing:-0.02em; }
.auth-gradient-text { background:linear-gradient(90deg, var(--primary) 0%, var(--secondary) 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; }
.auth-subtitle { margin:0; font-size:14px; color:var(--on-surface-variant); max-width:280px; margin:0 auto; line-height:1.5; }

.form-box { width:100%; background:rgba(255,255,255,0.85); backdrop-filter:blur(16px); padding:20px; border-radius:24px; box-shadow:0 4px 20px rgba(0,0,0,0.06); border:1px solid rgba(255,255,255,0.6); display:flex; flex-direction:column; gap:16px; margin-bottom:20px; }
...
.primary-btn { width:100%; padding:14px; border-radius:14px; background:linear-gradient(90deg, var(--primary), var(--secondary)); color:white; font-size:15px; font-weight:700; border:none; cursor:pointer; display:flex; align-items:center; justify-content:center; gap:8px; box-shadow:0 4px 14px rgba(70,72,212,0.3); transition:transform 0.15s; }
.primary-btn:active { transform:scale(0.98); }
.primary-btn:disabled { opacity:0.5; cursor:not-allowed; }

.secondary-btn { width:100%; padding:12px; border-radius:14px; background:var(--surface-container-low); color:var(--on-surface); font-size:14px; font-weight:600; border:none; cursor:pointer; }
